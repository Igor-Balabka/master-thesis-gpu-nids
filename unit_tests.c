...
long run_gpu_test_single_packet(AC_Automata *m, const char *data) {
    ...
    // Run benchmark on GPU and free host memory
    run_classic_packet_benchmark(m, &temp_pool, 1, &matches);
    cudaFreeHost(pinned_data);
    return matches;
}

// Unit test to check IP/TCP header parsing offsets
...
// Unit test to check Aho-Corasick detection on CPU
...
// Unit test to check identity/consistency between CPU and GPU results
