...
//Check the capacity of the automata, if it's to low then double it
...
//adding id for the rule and check if it's not duplicated
...
//add rules that we have to find
...
void ac_finalize(AC_Machine *m) {
    ...
    // BFS (Breadth-First Search) to build failure links
    while (head < tail) {
        int state = q[head++];
        ...
        for (int ch = 0; ch < ALPHABET_SIZE; ++ch) {
            ...
            if (next_state != -1) {
                ...
                // Optimization: Dense DFA (fill missing transitions)
