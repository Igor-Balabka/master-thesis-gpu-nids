    ...
    long * d_results_at_offset) 
{
    ...
    for (int j = 0; j < len; j++) {
        ...
        state = __ldg(&d_table[state * 256 + c]); //optimisation, usage of __ldg
    ...
    int threads_per_block, int batch_size) 
{
    ...
    char *pinned_payload;
    CUDA_CHECK(cudaHostAlloc(&pinned_payload, total_data_size, cudaHostAllocDefault));
    memcpy(pinned_payload, payload, total_data_size);
    ...
    for (int l = 0; l < loops; l++) {
        ...
        for (int i = 0; i < num_packets; i += batch_size) { 
            ...
            size_t batch_end = (size_t)packet_start[last_idx_in_batch] + (size_t)packet_lengths[last_idx_in_batch];
